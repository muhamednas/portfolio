      ...
      <div className={"relative overflow-hidden rounded-[2.5rem] p-10 md:p-16 border " + (isDark ? "bg-gradient-to-br from-violet-950/40 via-slate-900/60 to-cyan-950/40 border-white/10" : "bg-gradient-to-br from-violet-50 via-white to-cyan-50 border-slate-200")}>
        ...
        <div className="relative z-10 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h2 className={"text-5xl md:text-7xl font-black leading-[0.95] mb-6 " + (isDark ? "text-white" : "text-slate-900")}>Let us build something real.</h2>
            <p className={"text-lg font-light mb-8 " + (isDark ? "text-slate-300" : "text-slate-600")}>Open to internships, freelance AI projects, and technical co-founder conversations.</p>
            <a href={"mailto:" + PROFILE.email} className="inline-flex items-center gap-3 px-7 py-4 rounded-full font-bold bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white">
          ...
          <div className="space-y-3">
            {[
              ...
              const inner = (
                <>
                  <div className={"w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 " + (isDark ? "bg-white/10" : "bg-white border border-slate-200")}>
                    <Icon size={18} className={isDark ? "text-violet-300" : "text-violet-600"} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className={"text-xs font-semibold uppercase " + (isDark ? "text-slate-500" : "text-slate-500")}>{it.label}</div>
                    <div className={"font-semibold truncate " + (isDark ? "text-white" : "text-slate-900")}>{it.value}</div>
                  </div>
                  {it.href && <ArrowUpRight size={16} className={isDark ? "text-slate-400" : "text-slate-400"} />}
                </>
              );
              return it.href ? (
                <a key={i} href={it.href} target={it.href.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer" className={"flex items-center gap-4 p-4 rounded-2xl border " + (isDark ? "bg-white/5 border-white/10 hover:bg-white/10" : "bg-white/60 border-slate-200 hover:bg-white")}>
                  {inner}
                </a>
              ) : (
                <div key={i} className={"flex items-center gap-4 p-4 rounded-2xl border " + (isDark ? "bg-white/5 border-white/10" : "bg-white/60 border-slate-200")}>{inner}</div>
              );
            })}
          </div>
        </div>
