...
import { Github, Linkedin, ArrowUpRight, Brain } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto w-full grid lg:grid-cols-12 gap-12 items-center">
        ...
        <motion.div variants={container} initial="hidden" animate="visible" className="lg:col-span-7 order-2 lg:order-1">
          <motion.div variants={item} className="mb-6">
            <span className={"inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold uppercase " +
              (isDark ? "bg-violet-500/10 text-violet-300 border border-violet-400/20" : "bg-violet-100 text-violet-700 border border-violet-200")}>
          ...
          <motion.div variants={item} className="flex flex-wrap gap-3">
            ...
            <a
              href={`${import.meta.env.BASE_URL}MohamedNasir_CV.pdf`}
              target="_blank"
              rel="noopener noreferrer"
              download
              className={"flex items-center gap-2 px-7 py-4 rounded-full font-semibold text-sm transition-all " +
                (isDark ? "bg-white/5 text-white border border-white/10 hover:bg-white/10" : "bg-slate-900/5 text-slate-900 border border-slate-900/10 hover:bg-slate-900/15")}
            >
              Download CV
            </a>
            <a href={PROFILE.github} target="_blank" rel="noopener noreferrer"
              className={"flex items-center gap-2 px-7 py-4 rounded-full font-semibold text-sm " +
                (isDark ? "bg-white/5 text-white border border-white/10 hover:bg-white/10" : "bg-slate-900/5 text-slate-900 border border-slate-900/10 hover:bg-slate-900/15")}>
        ...
        <motion.div
          ...
          className="lg:col-span-5 order-1 lg:order-2 flex flex-col items-center lg:items-end gap-5"
        >
          {/* Profile image with floating badge */}
          <div className="relative">
            <div className="absolute -inset-4 bg-gradient-to-br from-violet-500 via-fuchsia-500 to-cyan-400 rounded-[2rem] blur-2xl opacity-40 animate-pulse" />
            <div className="absolute -inset-1 bg-gradient-to-br from-violet-500 via-fuchsia-500 to-cyan-400 rounded-[2rem] blur-md opacity-70" />
            <div className="relative w-72 h-80 md:w-80 md:h-96 rounded-[1.8rem] overflow-hidden bg-gradient-to-br from-violet-500 via-fuchsia-500 to-cyan-400 p-[2px]">
              <div className={"w-full h-full rounded-[1.7rem] overflow-hidden " + (isDark ? "bg-slate-900" : "bg-slate-100")}>
                <img src={PROFILE.photo} alt={PROFILE.fullName} className="w-full h-full object-cover" loading="eager" />
              </div>
            </div>
            ...
            <motion.div
              ...
              className={"absolute -top-4 -left-6 px-4 py-2.5 rounded-2xl border shadow-2xl " +
                (isDark ? "bg-white/10 border-white/20 text-white backdrop-blur-sm" : "bg-white/90 border-slate-200 text-slate-900")}
            >
